...
import { MapPin, Plus, CreditCard, ShoppingBag, ChevronRight, Check, Tag, X } from 'lucide-react';
...
export default function CheckoutPage() {
  ...
  const handleApplyCoupon = async () => {
    ...
    if (!totals) return;
    try {
      const result = await validateCoupon({ code: couponCode.trim(), cartSubtotal: totals.subtotal }).unwrap();
            ...
            <div className="card p-6 sticky top-20">
              ...
              <div className="pt-4 border-t border-gray-100 dark:border-dark-600">
                <p className="text-sm font-medium text-gray-700 dark:text-gray-200 mb-2 flex items-center gap-1.5">
                  <Tag className="w-3.5 h-3.5 text-primary-500" /> Have a coupon?
                </p>
                {appliedCoupon ? (
                  <div className="flex items-center justify-between bg-green-50 dark:bg-green-500/10 border border-green-200 dark:border-green-500/30 rounded-xl px-3 py-2">
                    <div>
                      <span className="text-xs font-bold text-green-600 dark:text-green-400 tracking-wider">{appliedCoupon.code}</span>
                      <p className="text-xs text-green-600 dark:text-green-400">−{formatCurrency(appliedCoupon.discountAmount)} saved!</p>
                    </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      {isValidating ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> : 'Apply'}
                    </button>
                  </div>
                )}
              </div>
              ...
              {totals && (
                <div className="space-y-2 pt-4 border-t border-gray-100 dark:border-dark-600 text-sm">
                  ...
                  <div className="flex justify-between font-bold text-gray-900 dark:text-white text-base pt-2 border-t border-gray-200 dark:border-dark-600">
                    ...
                    <span className="text-primary-500">{formatCurrency(finalGrandTotal)}</span>
                  </div>
                </div>
              )}
